...
class Encoder(Model):
    def __init__(self, latent_size):
        ...
        self.conv1 = L.Conv2d(1, 32, kernel_size=3, stride=1, pad=1)
        self.conv2 = L.Conv2d(32, 64, kernel_size=3, stride=2, pad=1)
        self.conv3 = L.Conv2d(64, 64, kernel_size=3, stride=1, pad=1)
        self.conv4 = L.Conv2d(64, 64, kernel_size=3, stride=1, pad=1)
        self.linear1 = L.Linear(12544, 32)
        self.linear2 = L.Linear(32, latent_size)
        self.linear3 = L.Linear(32, latent_size)
    ...
    def sampling(self, z_mean, z_log_var):
        ...
        xp = dazero.cuda.get_array_module(z_mean.data)
...
class Decoder(Model):
    def __init__(self):
        ...
        self.linear = L.Linear(2, np.prod(self.to_shape))
        self.deconv = L.Deconv2d(64, 32, kernel_size=4, stride=2, pad=1)
        self.conv = L.Conv2d(32, 1, kernel_size=3, stride=1, pad=1)
...
def show_digits(epoch=0):
    ...
    for i, yi in enumerate(grid_x):
        for j, xi in enumerate(grid_y):
            ...
            if use_gpu:
                z_sample = dazero.cuda.as_cupy(z_sample)
            with dazero.no_grad():
